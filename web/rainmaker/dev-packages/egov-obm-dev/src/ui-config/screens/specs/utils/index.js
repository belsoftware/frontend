...
// Converts UTC epoch value to IST Date and Time
export const convertEpochToDateWithTimeIST = dateEpoch => {
  let dateFromApi = new Date(dateEpoch);
  dateFromApi.setHours(dateFromApi.getHours() + 5);
  dateFromApi.setMinutes(dateFromApi.getMinutes() + 30);  
  let month = dateFromApi.getMonth() + 1;
  let day = dateFromApi.getDate();
  let year = dateFromApi.getFullYear();
  let time = dateFromApi.toLocaleTimeString();
  month = (month > 9 ? "" : "0") + month;
  day = (day > 9 ? "" : "0") + day;
...
export const setDocumentsInfo = async (state, dispatch) => {
  let applicationDocuments = get(
    state.screenConfiguration.preparedFinalObject,
    "chb.booking[0].applicationDocuments",
    []
  );
